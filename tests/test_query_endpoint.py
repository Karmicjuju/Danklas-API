        ...
        "citations": [
            {
                "retrievedReferences": [
                    {
                        "location": {
                            "s3Location": {"uri": "s3://bucket/document1.pdf"}
                        }
                    }
                ]
            },
            {
                "retrievedReferences": [
                    {
                        "location": {
                            "s3Location": {"uri": "s3://bucket/document2.pdf"}
                        }
                    }
                ]
            }
...
def test_query_knowledge_base_success(mock_bedrock_client, test_client, mock_bedrock_response):
    ...
    with patch("app.main.DANKLAS_ENV", "prod"):
        with patch("app.main.verify_jwt") as mock_verify_jwt:
            ...
            response = test_client.post(f"/knowledge-bases/{kb_id}/query", json=payload, headers=headers)
...
@patch("app.main.bedrock_client")
def test_query_knowledge_base_with_metadata_filters(mock_bedrock_client, test_client, mock_bedrock_response):
    ...
    filter_config = call_args["retrieveAndGenerateConfiguration"]["knowledgeBaseConfiguration"]["retrievalConfiguration"]["vectorSearchConfiguration"]["filter"]
...
def test_query_knowledge_base_bedrock_error(mock_bedrock_client, test_client):
    ...
    mock_bedrock_client.retrieve_and_generate.side_effect = Exception("Bedrock API error")
...
def test_build_metadata_filter_basic():
    ...
    tenant_filter = next(f for f in filters if f.get("equals", {}).get("key") == "tenant_id")
    assert tenant_filter["equals"]["value"] == "acme-corp"
    
    # Should have access level filter for non-admin
    access_filter = next(f for f in filters if f.get("equals", {}).get("key") == "access_level")
...
def test_build_metadata_filter_admin():
    ...
    access_filters = [f for f in filters if f.get("equals", {}).get("key") == "access_level"]
...
def test_build_metadata_filter_with_department():
    ...
    dept_filter = next(f for f in filters if f.get("equals", {}).get("key") == "department")
