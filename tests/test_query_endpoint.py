...
def test_audit_logging_request_start(test_client, caplog):
    ...
    for record in caplog.records:
        if (
            record.name == "danklas.audit"
            and getattr(record, "audit_type", "") == "request_start"
        ):
...
def test_audit_logging_request_success(test_client, caplog):
    ...
    for record in caplog.records:
        if (
            record.name == "danklas.audit"
            and getattr(record, "audit_type", "") == "request_success"
        ):
...
def test_audit_logging_structure(test_client, caplog):
    ...
    assert (
        len(audit_records) >= 2
    ), "Should have at least request_start and request_completed audit logs"
    ...
    for record in audit_records:
        ...
        assert record.audit_type in [
            "request_start",
            "request_success",
        ], f"Unexpected audit_type: {record.audit_type}"
...
def test_tracing_integration_query_endpoint(test_client):
    ...
    data = response.json()
    assert (
        f"Mock answer for KB {kb_id} and query '{payload['query']}'" in data["answer"]
    )
...
def test_tenant_mapping_in_logs(test_client, caplog, monkeypatch):
    ...
    monkeypatch.setenv(
        "DANKLAS_ENV", "dev"
    )  # Use dev mode for auth bypass but still test logging
...
def test_usage_tiers_structure():
    ...
    for tier_name, tier_config in USAGE_TIERS.items():
        for field in required_fields:
            ...
            assert isinstance(
                tier_config[field], int
            ), f"{field} should be integer in {tier_name} tier"
...
def test_guardrail_validation():
    ...
    invalid_content_filter = {
        ...
        "output_filters": {},
        "query_filters": {},
        "bedrock_config": {"model_settings": {}, "retrieval_config": {}},
    }
