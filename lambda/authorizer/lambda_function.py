...
def extract_user_context(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    dept_id = (
        payload.get('dept_id') or
        payload.get('custom:dept_id') or 
        payload.get('deptId') or
        payload.get('org', {}).get('id') if isinstance(payload.get('org'), dict) else None
    )
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Authorization successful for user: {user_context.get('user_id')} in department: {user_context.get('dept_id')}")
        return policy
