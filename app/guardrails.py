...
DEFAULT_GUARDRAIL = {
    ...
    "bedrock_config": {
        "model_settings": {"temperature": 0.1, "top_p": 0.9, "max_tokens": 2048},
        "retrieval_config": {"number_of_results": 5, "search_type": "HYBRID"},
    },
}
...
class GuardrailManager:
    ...
    def _initialize_ssm_client(self):
        ...
        try:
            ...
            logger.warning(
                f"Failed to initialize SSM client: {e}, using default guardrail"
            )
            self.ssm_client = None
    ...
    def get_guardrail(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if guardrail:
            ...
            logger.info(
                f"Guardrail loaded and cached with checksum: {self._cached_checksum}"
            )
            return guardrail
        else:
    ...
    def validate_guardrail(self, guardrail_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if (
                "model_settings" not in bedrock_config
                or "retrieval_config" not in bedrock_config
            ):
