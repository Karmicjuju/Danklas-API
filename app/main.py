...
from app.tracing import configure_tracing, create_span, get_tracer, instrument_fastapi
...
)  # In production, this would be CloudWatch handler
...
async def health_check():
    ...
    try:
        ...
        health_status["guardrails"] = {"status": "degraded", "error": str(e)}
    ...
    try:
        ...
        health_status["rate_limiting"] = {"status": "error", "error": str(e)}
    ...
    try:
        ...
        health_status["tracing"] = {"status": "error", "error": str(e)}
...
async def get_tenant_usage_stats(request: Request):
    ...
    return {"tenant_id": tenant_id, "usage_statistics": stats}
...
async def get_guardrails_info(request: Request):
    ...
    return {"guardrail_info": info, "loaded_at_startup": True, "cache_enabled": True}
...
async def get_guardrails_config(request: Request):
    ...
    return {"guardrail_configuration": guardrail, "metadata": get_guardrail_info()}
...
async def query_knowledge_base(
    ...
    with create_span("bedrock.knowledge_base.query") as span:
        ...
        span.set_attribute(
            "kb.tenant_id", getattr(request.state, "tenant_id", "unknown")
        )
    ...
    with create_span("bedrock.knowledge_base.status") as span:
        ...
        try:
            ...
            response = StatusResponse(
                status="READY", lastSyncedAt="2024-07-25T12:00:00Z"
            )
...
@app.post(
    "/knowledge-bases/{kb_id}/refresh", response_model=RefreshResponse, status_code=202
)
    ...
    with create_span("bedrock.knowledge_base.refresh") as span:
        ...
        try:
            ...
            response = RefreshResponse(
                jobId=job_id, message=f"Refresh started for KB {kb_id}"
            )
